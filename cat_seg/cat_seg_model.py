...
#Detctron2 is an open source software project that facilitates computer vision research and development.
...
class vCATSeg(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.clip_resolution = (224, 224) if clip_pretrained == "google/siglip-base-patch16-224" else (224, 224)

        self.proj_dim = 768 if clip_pretrained == "google/siglip-base-patch16-224" else 1024
        ...
        self.layer_indexes = [3, 7] if clip_pretrained == "google/siglip-base-patch16-224" else [7, 15] 
        self.layers = []
    ...
    def forward(self, batched_inputs):
        ...
        res3 = rearrange(image_features, "B (H W) C -> B C H W", H=14)
    ...
    @torch.no_grad()
    def inference_sliding_window(self, batched_inputs, kernel=224, overlap=0.333, out_res=[640, 640]):
        ...
        inputs = self.sem_seg_head.predictor.processor(images=clip_images_resized, return_tensors="pt").to(self.device)
        clip_features = self.sem_seg_head.predictor.clip_model.get_image_features(**inputs)
        res3 = rearrange(clip_features[:, 1:, :], "B (H W) C -> B C H W", H=14)
