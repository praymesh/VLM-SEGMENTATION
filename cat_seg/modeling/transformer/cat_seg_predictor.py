...
""" We are not using the CLIP model, thus we need to replace all the CLIP specific imports.
clip.py contains the CLIP model and its tokenizer and transforms.
from cat_seg.third_party import clip
instead we use transformers libray for SIGLIP2(https://huggingface.co/google/siglip2-base-patch16-224/tree/main)"""
...
class CATSegPredictor(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.clip_model = SiglipModel.from_pretrained("google/siglip-base-patch16-224").to(device)
        ...
        self.text_features = self.class_embeddings(self.class_texts, prompt_templates, self.clip_model).permute(1, 0, 2).float()
        self.text_features_test = self.class_embeddings(self.test_class_texts, prompt_templates, self.clip_model).permute(1, 0, 2).float()
    ...
    def class_embeddings(self, classnames, templates, clip_model):
        ...
        for classname in classnames:
            ...
            if self.tokenizer is not None:
                ...
                inputs = self.processor(text=texts, return_tensors="pt", padding=True).to(self.clip_model.device)
    ...
    def class_embeddings(self, classnames, templates, clip_model):
        ...
        for classname in classnames:
            ...
            # Use SIGLIP2 processor and model
            inputs = self.processor(text=texts, return_tensors="pt", padding=True).to(self.clip_model.device)
            class_embeddings = self.clip_model.get_text_features(**inputs)
            class_embeddings = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)
            if len(templates) != class_embeddings.shape[0]:
            ...
            class_embedding = class_embeddings
            zeroshot_weights.append(class_embedding)
        zeroshot_weights = torch.stack(zeroshot_weights, dim=1).to(self.clip_model.device)
        return zeroshot_weights
